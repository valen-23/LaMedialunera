<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" href="./logopag/logopag.ico">
    <title>Contactos</title>
    <link rel="stylesheet" href="./estilos.css"></script>
    <link rel="stylesheet" href="style.css">
    <link href="./bootstrap-3.4.1-dist/css/bootstrap.min.css">
     <!--[if lt IE 9]-->
    <script scr="https:://oss.maxcdn.com/libs/html5shiv/3.4.1/html5shiv.js"></script>
    <script scr="https:://oss.maxcdn.com/libs/respond.js/1.4.2/respond.min.js"></script>
    <script scr="./bootstrap-3.4.1-dist\js\bootstrap.min.js"></script>
    <script src="http://code.jquery.com/jquery-latest.js"></script>
</head>

<body>
    <header>
        <div class="menubar">
          <a href="#" class="bt-menu"><span class="icon-menu"></span><img src="img/Scan5.png" class="imagendecabeza"></a>
        </div>
        <nav>
           <ul>
              <li class="lidif" href=""><img src="./img/Scan5.png" class="imagendecabeza"></a></li>
              <li class="lif" id="lif2"><a href="./pedidos.php"><span class="icon-clipboard"></span>Realiza tu Pedido</a></li>
              <li class="lif" id="lif1"><a href="./zonasdeentrega.php"><span class="icon-location2"></span>Zona de Entrega</a></li>
              <li class="lif" id="lif1"><a href="./contactos.php"><span class="icon-phone"></span>Contacto</a></li>
              <li class="lif" id="lifhome"><a href="./index.php"><span class="icon-home"></span>Inicio</a></li>
           </ul>
         </nav>
      </header>

        <script src="http://code.jquery.com/jquery-latest.js"></script>
        <script src="js/menu.js"></script>

    <br>
    <br>
        <!--CONTACTOS-->
    <div class="contactos">
        <h2 id="contactosalign"><span class="icon-address-book"></span>Contactos:</h2>
        <br>
        <br>
        <br>
        <br>
            <a href="" title="NUMERO DE TELEFONO"><img src="img/icontel2.png" class="icontel"></a>
        <br>
        <br>
        <hr>
        <br>
        <br>
            <a href="mailto:paganivalentino06@gmail.com" title="GMAIL"><img src="img/icongmail.png" class="icongmail"></a>
    </div>
</body>
</html>
